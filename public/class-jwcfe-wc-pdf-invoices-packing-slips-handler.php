...
if(!class_exists('JWCFE_WC_PDF_Invoices_Packing_Slips_Handler')):

class JWCFE_WC_PDF_Invoices_Packing_Slips_Handler extends JWCFE_Checkout_Fields_Utils{
	...
	public function is_enabled($position){
		$enabled = $position === apply_filters('jwcfe_wpo_wcpdf_display_position', 'after_order_data') ? true : false;
		return $enabled; 
	}
	...
	public function display_custom_fields($template_type, $order, $display='') {
		...
		if($html){
			if($display === self::DISPLAY_ROW || $display === self::DISPLAY_CELL){
				...
				echo '<table class="jwcfe-wcpdf-table" style="margin-bottom: 20px;"><tbody>';
				echo $html;
				echo '</tbody></table>';
			}
		}
	...
	public function display_wcpdfips_custom_fields($order, $fields, $display='') {
		...
		if(is_array($fields) && !empty($fields)){
			...
			$is_nl2br = apply_filters('jwcfe_nl2br_custom_field_value', true);
			...
			if(is_array($fields_arr) && !empty($fields_arr)){
				...
				if($sections){
					foreach($sections as $sname => $section){	
						$fieldset = JWCFE_Utils_Section::get_fields($section);
						
						if($fieldset && is_array($fieldset)){
							foreach($fieldset as $key => $field){
								if(JWCFE_Utils_Field::is_custom_field($field) && JWCFE_Utils_Field::is_enabled($field) && in_array($key, $fields_arr)){
									$nfield = array();
