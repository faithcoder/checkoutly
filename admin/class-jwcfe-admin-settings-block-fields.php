	...
	class JWCFE_Admin_Settings_Block_Fields extends JWCFE_Admin
	{
        ...
        public function checkout_form_field_editor()
		{
            ...
            if ($section == 'account') {
                ...
                // Popup form is rendered inside jwcfe_field_popup() below
                ...
                echo '<div class="wrap woocommerce jwcfe-wrap"><div class="icon32 icon32-attributes" id="icon-woocommerce"><br /></div>';
                $this->render_page_header();
                // ══ ALL SECTIONS ON ONE PAGE (billing / shipping / additional) ══
                echo '<div class="wrap woocommerce jwcfe-wrap"><div class="icon32 icon32-attributes" id="icon-woocommerce"><br /></div>';
                $this->render_tabs_and_sections();
                ...
                $section_meta = array(
                    'billing'    => array(
                        'label' => __('Contact Information', 'jwcfe'),
                ...
                <form method="post" id="jwcfe_checkout_fields_form" action="">
                    ...
                    // section, every section overwrites the previous section's POST
                    // keys (f_name[0], f_type[0], etc.) and only the last section
                    // survives on save.
                    $i = 0;
                    ...
                    <table id="jwcfe_checkout_fields_<?php echo esc_attr($sec_key); ?>" class="wc_gateways widefat jwcfe-fields-table" cellspacing="0" data-section="<?php echo esc_attr($sec_key); ?>">
                        ...
                        <tbody class="ui-sortable" id="jwcfe_sortable_<?php echo esc_attr($sec_key); ?>">
                            <?php
                            // NOTE: $i is intentionally NOT reset here; it was
                            // declared before the outer loop so indices remain
                            // globally unique across all sections.
                            $tab = $this->get_current_tab();
                            ...
                            foreach ($sec_fields as $name => $options) :
                                ...
                                if (!isset($options['min_time']))    { $options['min_time'] = ''; }
                                if (!isset($options['max_time']))    { $options['max_time'] = ''; }
                                if (!isset($options['time_step']))   { $options['time_step'] = ''; }
                                if (!isset($options['time_format'])) { $options['time_format'] = ''; }
                                if (!isset($options['rules']))           { $options['rules'] = ''; }
                                if (!isset($options['rules_action']))    { $options['rules_action'] = ''; }
                                if (!isset($options['rules_ajax']))      { $options['rules_ajax'] = ''; }
                                if (!isset($options['rules_action_ajax'])){ $options['rules_action_ajax'] = ''; }
                                ...
                                $options['is_include']    = (isset($options['is_include']) && $options['is_include'] == 1) ? '1' : '0';
                                $options['access']        = (isset($options['access']) && $options['access'] == 1) ? '1' : '0';
                                $options['enabled']       = (!isset($options['enabled']) || $options['enabled'] == 1) ? '1' : '0';
                                if (!isset($options['type'])) { $options['type'] = 'text'; }
                                $options['show_in_email'] = (isset($options['show_in_email']) && $options['show_in_email'] == 1) ? '1' : '0';
                                $options['show_in_order'] = (isset($options['show_in_order']) && $options['show_in_order'] == 1) ? '1' : '0';

                                $disabled = ($name == 'billing_email' || $name == 'account_username' || $name == 'account_password');
                            ?>
                                <tr class="row_<?php echo $i; echo ($disabled ? ' jwcfe-disabled' : ($options['enabled'] == 1 ? '' : ' jwcfe-disabled')); ?>">
                                    <td width="1%" class="sort ui-sortable-handle">
                                        ...
                                        <input type="hidden" name="f_text[<?php echo $i; ?>]"         class="f_text"     value="<?php echo stripcslashes($options['text']); ?>" />
                                    ...
                                    <td class="td_type">
                                        ...
                                        $type_map = array(
                                            'select'=>'Select / Dropdown','multiselect'=>'Multi-Select',
                                            'radio'=>'Radio Button','checkbox'=>'Checkbox',
                                            'checkboxgroup'=>'Checkbox Group','date'=>'Date Picker',
                                            'timepicker'=>'Time Picker','week'=>'Week Picker',
                                            'month'=>'Month Picker','textarea'=>'Textarea',
                                    ...
                                    <td class="td_description"><?php echo esc_html(isset($options['text']) ? $options['text'] : ''); ?></td>
                                    <td class="td_validation"><?php echo esc_html(isset($options['validate']) ? $options['validate'] : ''); ?></td>
                                    ...
                                    <td class="td_edit">
                                        <div class="jwcfe-actions-cell">
                                            <?php if (!$disabled) : ?>
                                            <div class="jwcfe-icon-btn edit" onclick="openEditFieldForm(this,<?php echo $i; ?>)" title="<?php esc_attr_e('Edit field', 'jwcfe'); ?>">
        ...
        } // end checkout_form_field_editor()
                    ...
                    <div id="jwcfe_new_field_form_pp" class="<?php echo $addClass; ?> jwcfe_popup_wrapper">
                        <form method="POST" id="jwcfe_new_field_form" action="">
                            ...
                            <div class="jwcfe-modal-body" id="jwcfe_field_editor_form_new">
                                ...
                                <div class="jwcfe-modal-row-2col">
                                    <div class="jwcfe-modal-field rowfield">
                                        ...
                                        <select name="ftype" class="jwcfe-modal-select" onchange="jwcfeFieldTypeChangeListnerblock(this)">
                                            ...
                                            <option value="select"><?php esc_html_e('Select', 'jwcfe'); ?></option>
                                ...
                                <!-- Row 5: Validation -->
                                <div class="jwcfe-modal-row-1col rowValidate">
                                    <div class="jwcfe-modal-field">
                                        <label class="jwcfe-modal-label"><?php esc_html_e('Validation', 'jwcfe'); ?></label>
                                        <select multiple="multiple" name="fvalidate" class="jwcfe-enhanced-multi-select jwcfe-modal-select" data-placeholder="<?php esc_attr_e('Please Select', 'jwcfe'); ?>" style="height:42px;">
